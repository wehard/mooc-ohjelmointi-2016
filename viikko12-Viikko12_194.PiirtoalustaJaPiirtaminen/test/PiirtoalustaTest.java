...
public class PiirtoalustaTest {
    ...
    public static final String LUOKAN_NIMI =
            "piirtaminen.kayttoliittyma.Piirtoalusta";

    @Test
    public void onPiirtoalusta() {
        luoPiirtoalusta();
    }

    @Test
    public void kokoOikea() {
        Assert.assertTrue("et saa muuttaa luokan Kayttoliittyma sisältöä!",
                sisaltaa("newDimension(400,400)"));

    }

    @Test
    public void piirtoalustallaKaytetaanVainSovittujaMetodeja() {
        JPanel piirtoalusta = luoPiirtoalusta();
        ...
        for (Method m : piirtoalusta.getClass().getDeclaredMethods()) {
            ...
            try {
                ...
                Assert.fail("Metodia " + m.getName() + " ei voinut asettaa julkiseksi.");
            }
        ...
        try {
            paintCompoMethod.invoke(piirtoalusta, new PiirtoalustaTest.MockGraphics());
        } catch (UnsupportedOperationException e) {
            Assert.fail("Käytäthän luokan " + LUOKAN_NIMI + " metodissa paintComponent kutsun super.paintComponent(graphics) lisäksi vain graphics-olion metodia fillRect?");
        } catch (Exception e) {
            Assert.fail("Odottamaton virhe: " + e.getMessage());
        }
    }

    @Test
    public void kaytetaanFillRectMetodia() {
        JPanel piirtoalusta = luoPiirtoalusta();
        Method paintCompoMethod = null;
        for (Method m : piirtoalusta.getClass().getDeclaredMethods()) {
            if (!m.getName().equals("paintComponent")) {
        ...
        PiirtoalustaTest.MockGraphics graphics = new PiirtoalustaTest.MockGraphics();
        try {
        ...
        Assert.assertTrue("Käytäthän luokan " + LUOKAN_NIMI + " metodissa paintComponent metodia graphics.fillRect()?", graphics.rects.size() > 1);
    }

    @Test
    public void piirretaanHymio() {
        ...
        Assert.assertFalse("Piirräthän kuvaan jotain?", piirrettyMinimi == piirrettyMaksimi);
        ...
        for (int i = 0; i < koko * koko; i++) {
            ...
            if (PiirtoalustaTest.Kuva.data[x][y] == 1) {
                smileyTummia++;
            }

            if (PiirtoalustaTest.Kuva.data[x][y] == 1 && piirrettyMinimi == piirrettyValue) {
                samojaTummia++;
                continue;
            }

            if (PiirtoalustaTest.Kuva.data[x][y] == 0) {
                smileyVaaleita++;
            }

            if (PiirtoalustaTest.Kuva.data[x][y] == 0 && piirrettyMaksimi == piirrettyValue) {
                samojaVaaleita++;
                continue;
            }
        }
        ...
        Assert.assertTrue("Kuvan vaaleiden kohtien pitää olla vähintään 80%:sti samanlainen kuin esimerkkikuvan. Esimerkkikuva löytyy projektin hakemistosta nimellä smiley.gif. Nyt kuvasi vaaleat osat olivat noin " + (vaaleistaOikein * 100) + " prosenttisesti oikein. ", vaaleistaOikein >= 0.8);
        Assert.assertTrue("Kuvan tummien kohtien pitää olla vähintään 80%:sti samanlainen kuin esimerkkikuvan. Esimerkkikuva löytyy projektin hakemistosta nimellä smiley.gif. Nyt kuvasi tummat osat olivat noin " + (tummistaOikein * 100) + " prosenttisesti oikein. ", tummistaOikein >= 0.8);

    }

    @Test
    public void hymioPiirretaanViidellaFillRectKutsulla() {
        JPanel piirtoalusta = luoPiirtoalusta();
        Method paintCompoMethod = null;
        for (Method m : piirtoalusta.getClass().getDeclaredMethods()) {
            if (!m.getName().equals("paintComponent")) {
        ...
        Assert.assertTrue("Käytäthän hymiön piirtämiseen viittä fillRect-kutsua?", graphics.rects.size() > 4 && graphics.rects.size() <= 6);
    }
    ...
    private JPanel luoPiirtoalusta() {
        Reflex.ClassRef<?> luokka;
        try {
            luokka = Reflex.reflect(LUOKAN_NIMI);
        } catch (Throwable t) {
            Assert.fail("Luokkaa " + LUOKAN_NIMI + " ei ole olemassa. Tässä tehtävässä täytyy luoda kyseinen luokka.");
            return null;
        }
        ...
        if (!JPanel.class.isAssignableFrom(
            ...
            Assert.fail(
                    "Luokan " + LUOKAN_NIMI + " täytyy "
                    + "periä luokka " + JPanel.class.getName());
        }
        ...
        try {
            ...
            Assert.fail("Luokalla " + LUOKAN_NIMI + " ei määrettä public tai sillä ei ole julkista parametritonta konstruktoria.");
            return null;
        }
    ...
    private boolean sisaltaa(String kutsu) {
        ...
        try {
            Scanner lukija = new Scanner(new File("src/piirtaminen/kayttoliittyma/Kayttoliittyma.java"));
