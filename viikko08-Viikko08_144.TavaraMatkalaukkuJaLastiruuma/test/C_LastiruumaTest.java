...
public class C_LastiruumaTest<_Tavara, _Matkalaukku, _Lastiruuma> {
    ...
    @Test
    @Points("144.8")
    public void lastiruumaTiiliskivillaTarkistaTulostus() {
        MockInOut io = new MockInOut("");
        Method lisaaTiiliskiviaRuumaanMeto = null;
        try {
            lisaaTiiliskiviaRuumaanMeto = ReflectionUtils.requireMethod(Main.class, "lisaaMatkalaukutTiiliskivilla", lastiruumaClass);
        } catch (Throwable t) {
            fail("Onhan luokassa Main metodi public static void lisaaMatkalaukutTiiliskivilla(Lastiruuma lastiruuma)?");
        }

        Object lastiruuma = luoRuuma(3);
        try {
            lisaaTiiliskiviaRuumaanMeto.invoke(null, lastiruuma);
        } catch (Throwable ex) {
            fail("Tarkista että luokassa Main on metodi public static void lisaaMatkalaukutTiiliskivilla(Lastiruuma lastiruuma), ja että se yrittää lisätä 100 tiiliskivellistä matkalaukkua lastiruumaan.");
        }

        Method toString = ReflectionUtils.requireMethod(lastiruumaClass, "toString");
        try {

            String palautus = ReflectionUtils.invokeMethod(String.class, toString, lastiruuma);

            if (!sisaltaa(palautus, "2", "matkalaukku", "3")) {
                fail("Varmista että Main-luokassa oleva metodisi lisaaMatkalaukutTiiliskivilla yrittää lisätä laukut siten, että ensin lisätään laukku jossa kilon painoinen tiiliskivi, seuraavaksi kahden kilon painoinen tiiliskivi, ...");
            }
        } catch (Throwable t) {
            fail(t.getMessage());
        }

        lastiruuma = luoRuuma(2000);
        try {
            lisaaTiiliskiviaRuumaanMeto.invoke(null, lastiruuma);
        } catch (Throwable ex) {
            fail("Tarkista että luokassa Main on metodi public static void lisaaMatkalaukutTiiliskivilla(Lastiruuma lastiruuma), ja että se yrittää lisätä 100 tiiliskivellistä matkalaukkua lastiruumaan.");
        ...
        try {
            ...
            String palautus = ReflectionUtils.invokeMethod(String.class, toString, lastiruuma);
            if (!sisaltaa(palautus, "62", "matkalaukku", "1953")) {
                fail("Varmista että Main-luokassa oleva metodisi lisaaMatkalaukutTiiliskivilla yrittää lisätä laukut siten, että ensin lisätään laukku jossa kilon painoinen tiiliskivi, seuraavaksi kahden kilon painoinen tiiliskivi, ... Jos lastiruumaan mahtuu 2000 kiloa, laukkuja pitäisi olla 62 -- yhteispaino 1953 kg.");
            }
        } catch (Throwable t) {
            fail(t.getMessage());
        }
